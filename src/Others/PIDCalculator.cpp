 ...
 *               The default is 50ms.
 */
PIDCalculator::PIDCalculator(double Kp, double Ki, double Kd, double Kf,
                             double period) {
 // m_controlLoop = std::make_unique<Notifier>(&PIDCalculator::Calculate, this);
 ...
 */
double PIDCalculator::CalculateFeedForward() {
  if (m_sourceType == PIDSourceType::kRate) {
 ...
 */
void PIDCalculator::SetPIDSourceType(PIDSourceType pidSource) {
 ...
 */
PIDSourceType PIDCalculator::GetPIDSourceType() const {
  return m_sourceType;
}
...
std::string PIDCalculator::GetSmartDashboardType() const {
  return "PIDController";
}

void PIDCalculator::InitTable(std::shared_ptr<ITable> subtable) {
  if (m_table != nullptr) m_table->RemoveTableListener(this);
  m_table = subtable;
  if (m_table != nullptr) {
 ...
 */
double PIDCalculator::GetContinuousError(double error) const {
  if (m_continuous) {
...
std::shared_ptr<ITable> PIDCalculator::GetTable() const { return m_table; }

void PIDCalculator::ValueChanged(ITable* source, llvm::StringRef key,
...
void PIDCalculator::UpdateTable() {}

void PIDCalculator::StartLiveWindowMode() { Disable(); }

void PIDCalculator::StopLiveWindowMode() {}
